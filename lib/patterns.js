  ...
  prompt
    ...
    .then (answers => {
      ...
      if (answers.updateIndexFile) {
        ...
        const fileName = answers.fileName.replace(answers.fileName.replace(/\.(js|jsx)$/, ''))
        const relativePath = answers.fileName.replace(relativePathToIndex.replace(/\.(js|jsx)$/, ''))

        const content = `import ${fileName} from '${relativePath}'`
